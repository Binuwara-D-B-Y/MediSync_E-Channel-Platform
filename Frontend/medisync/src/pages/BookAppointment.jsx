...
import '../styles/BookAppointment.css';
import ClientBookingModal from '../components/ClientBookingModal';
import { apiRequest, authHeaders } from '../api'; // ✅ import API helpers
...
export default function BookAppointment() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const foundDoctor = doctors.find(d => d.doctorId == doctorId);
        if (foundDoctor) {
          // Add missing fields for UI safety
          foundDoctor.consultationFee = foundDoctor.consultationFee || 2000;
          foundDoctor.profileImage = foundDoctor.profileImage || '/images/unnamed.png';
          foundDoctor.wardRoom = foundDoctor.wardRoom || 'A-101';
          foundDoctor.rating = foundDoctor.rating || 4.5;
      ...
      // Use mock data for now - replace with real schedule API later
      setAvailableSlots(mockTimeSlots);
      setLoading(false);
    };
  ...
  const getAvailableSlotsForDoctor = () =>
    (availableSlots || []).filter(s => s.doctorId == doctor?.doctorId || s.doctorId == doctor?.id);

  // Handle booking confirmation
  const handleConfirmBooking = (slot) => {
    if (!slot) return;
    setSelectedDate(slot.date || slot.day || '');
    setSelectedTime(slot.time || slot.start || '');
    setSelectedSlot(slot);
    setShowClientForm(true);
  };
  ...
  const handlePayNow = async (patient) => {
    ...
    // Save patient info for success screen
    setPatientName(patient.name);
    setPatientNIC(patient.nic);
    setPatientEmail(patient.email);
    setPatientContactNo(patient.contact);
      ...
      setAvailableSlots(prev =>
        prev.map(s =>
          s.id === selectedSlot.id
            ? { ...s, availableSlots: Math.max(0, (s.availableSlots || 0) - 1) }
            : s
        )
      );
    ...
    <div className="booking-page">
      <div className="booking-container">
        ...
        <div className="booking-content">
          ...
          <aside className="doctor-profile">
            <div className="doctor-image">
              <img src={doctor.profileImage} alt={doctor.fullName} onError={(e) => { e.currentTarget.src = '/images/unnamed.png'; }} />
            </div>
            <div className="doctor-info">
              <h2>{doctor.fullName}</h2>
              <div className="specialization">{doctor.specialization}</div>
          ...
          <div className="booking-form schedule-panel">
            <h3>Available Schedule</h3>
            <div className="schedule-list">
              {getAvailableSlotsForDoctor().length === 0 && <div className="no-slots">No available slots.</div>}
              {getAvailableSlotsForDoctor().map(slot => (
                <div key={slot.id} className="slot-card">
                  <div className="slot-meta">
                    <div className="slot-row">
        ...
        {showClientForm && selectedSlot && (
          <ClientBookingModal
            doctor={doctor}
            slot={selectedSlot}
            onClose={handleCancelClientForm}
            onPay={handlePayNow}
            isProcessing={isBooking && bookingSlotId === selectedSlot.id}
          />
        )}
