...
const AdminDoctors = () => {
    ...
    const handleDelete = async (doctorId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`/api/admin/admindoctors/${doctorId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
                    ...
                    <tbody>
                        {doctors.map(doctor => (
                            <tr key={doctor.doctorId}>
                                ...
                                <td title={doctor.details}>{doctor.details.length > 50 ? doctor.details.substring(0, 50) + '...' : doctor.details}</td>
                                <td>{doctor.email}</td>
