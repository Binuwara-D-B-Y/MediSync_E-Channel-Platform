      ...
      try {
        if (hasName && hasSpec) {
          const [nameData, specData] = await Promise.all([
            doctorsAPI.getByName(searchTerm),
            doctorsAPI.getBySpecialization(selectedSpecialization)
          ]);
          ...
          let data = await doctorsAPI.getByName(searchTerm);
          if (!data || data.length === 0) {
          ...
          const data = await doctorsAPI.getBySpecialization(selectedSpecialization);
          setDoctorsBySpec(data || []);
          setDoctors(data || []);
          setDisplayMode('specOnly');
        } else {
